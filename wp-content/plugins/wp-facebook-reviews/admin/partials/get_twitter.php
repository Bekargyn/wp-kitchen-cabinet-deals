	...
	if (isset($_POST['wprevpro_submittemplatebtn'])){
		...
		if($t_id==""){
			...
			if($updatetempquery>0){
				$dbmsg = $dbmsg.'<div id="setting-error-wprevpro_message" class="updated settings-error notice is-dismissible">'.__('<p><strong>Form Updated!</strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button>', 'wp-review-slider-pro').'</div>';
			} else {
				$dbmsg = $dbmsg.'<div id="setting-error-wprevpro_message" class="error settings-error notice is-dismissible">'.__('<p><strong>Oops! The Form could not be updated in the database. </strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button>', 'wp-review-slider-pro').'</div>';
			}
		}
...
<div class='bordereddiv'>Use this page to search for and download tweets about your business/product/service. Click the <b>"Add New Twitter Source"</b> button to get started. If you need to search for tweets older than 7 days, or you get API Key errors, then you will need to use the <b>"Enter/Modify API Keys"</b> button to enter your own keys.</div>
...
<div id="apikeyformdiv" class="<?php echo $keyformhideshow; ?> wprevpro_margin10 bordered_form" id="login_cookie">
		...
		<tbody>
			<tr class="wprevpro_row">
				...
				<div class="twitter_key_header">Consumer API Keys:</div>
				<div class="twitter_key_div">API Key: <input class="inputrow100per" type="text" name="wprevpro_twitter_api_key" id="wprevpro_twitter_api_key" value="<?php echo get_option('wprevfb_twitterapi_key'); ?>"></div>
				<div class="twitter_key_div">API Secret Key: <input class="inputrow100per" type="text" name="wprevpro_twitter_api_key_secret" id="wprevpro_twitter_api_key_secret" value="<?php echo get_option('wprevfb_twitterapi_key_secret'); ?>"></div>
				</td>
				<td scope="row" style="padding-left:10px;">
				<div class="twitter_key_header">Access token & access token secret:</div>
				<div class="twitter_key_div">Access Token: <input class="inputrow100per" type="text" name="wprevpro_twitter_api_token" id="wprevpro_twitter_api_token" value="<?php echo get_option('wprevfb_twitterapi_token'); ?>"></div>
				<div class="twitter_key_div">Access Token Secret: <input class="inputrow100per" type="text" name="wprevpro_twitter_api_token_secret" id="wprevpro_twitter_api_token_secret" value="<?php echo get_option('wprevfb_twitterapi_token_secret'); ?>"></div>
				</td>
			</tr>
			<tr class="wprevpro_row">
...
&nbsp;&nbsp;<a href="https://ljapps.com/how-to-get-your-twitter-api-keys-access-tokens-access-premium-search-api/" target="_blank" id="instr" name="instr" class="button-secondary ">API Instructions</a>
	...
	<table class="wprevpro_margin10 form-table ">
		<tbody>
			...
			<tr class="wprevpro_row">
				...
				<td>
					<input type="radio" name="wprevpro_profile_img" value="no" <?php if($currentgetappform->profile_img=='no' || $currentgetappform->profile_img==''){echo "checked";} ?>>No&nbsp;&nbsp;&nbsp;
					<input type="radio" name="wprevpro_profile_img" value="yes" <?php if($currentgetappform->profile_img=='yes' ){echo "checked";} ?>>Yes&nbsp;&nbsp;&nbsp;
					<p class="description">
...
echo $dbmsg;
	...
	$urltrimmed = remove_query_arg( array('taction', 'id') );
		...
		$html .= '<tr id="'.$currentform->id.'">
				...
				<th scope="col" class="manage-column" limage="'.$currentform->profile_img.'" fcats="'.$fcategories.'" fposts="'.$fposts.'" ftitle="'.$currentform->title.'" epoint="'.$currentform->endpoint.'" squery="'.$currentform->query.'"><a href="'.$url_tempeditbtn.'" class="rfbtn button button-secondary dashicons-before dashicons-admin-generic">'.__('Edit', 'wp-review-slider-pro').'</a> <a href="'.$url_tempdelbtn.'" class="rfbtn button button-secondary dashicons-before dashicons-trash">'.__('Delete', 'wp-review-slider-pro').'</a> <a href="'.$url_tempcopybtn.'" class="rfbtn button button-secondary dashicons-before dashicons-admin-page">'.__('Copy', 'wp-fb-reviews').'</a> <span class="rfbtn button button-primary dashicons-before dashicons-star-filled retreviewsbtn"> '.__('Get Tweets', 'wp-fb-reviews').'</span></th>
			</tr>';
	}
	} else {
...
<div id="retreivewspopupdiv" style="display:none;">
	...
	<input id="tb_content_query_input" data-custom="custom" type="text" name="tb_content_query_input" value="">&nbsp;<span class="button button-secondary updatequery">Update</span>
	</div>
